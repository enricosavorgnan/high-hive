...
namespace Hive {
    ...
    class RuleEngine {
        public:
            ...
            static bool canSlide(const Board& board, Coord from, Coord target, std::optional<Coord> ignoreCoord = std::nullopt);
            ...
            // Method that defines whether the move from exclude coordinate to target coordinates does not break the One Hive Rule, leaving the piece in target coordinate far from other pieces.
            static bool touchesHive(const Board& board, Coord target, Coord exclude);

            // Precomputes the articulation points using Tarjan's Algorithm.
